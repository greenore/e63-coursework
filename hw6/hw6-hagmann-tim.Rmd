...
The SLF4J error can be ignored. However, if the number of standard deviations by which a task's average progress-rates is lower than the average of all running tasks the so called *race condition* is appearing. That means that by copying irectly from the local file system to the hadoop *input* directory, from local files there are cases where Spark tries to read the file just after it's name was changes. This leas to files named chunkaa._COPYING_". When the file is finished copying, the extension is changed. If Spark now sees the new temp file and then tries to read it just after its name was changed, it causes a File not Found Error and the scripts exits. That means that once one of the two tasks finishes successfully, the other task is killed.
...
mkdir -p /home/tim/e63-coursework/hw6/data/output
mkdir -p /home/tim/e63-coursework/hw6/data/staging
```

### Move generated chunks to local file system
```{bash eval=FALSE}
...
```{bash eval=FALSE}
spark-submit --master local[2] /home/tim/e63-coursework/hw6/scripts/count-buys2.py
```
...
```{bash eval=FALSE}
spark-submit --master local[2] /home/tim/e63-coursework/hw6/scripts/stateful_wordcount.py localhost 9999
```

```{r}
spark-submit /home/tim/spark/examples/src/main/python/streaming/stateful_network_wordcount.py localhost 9999
```
...
positive examples - words beginning with "a" or "b"
negative examples - not the above