...
df_ulysses_cleaned = df_ulysses_cleaned.select(removePunctuation(col('words')))
df_ulysses_cleaned = df_ulysses_cleaned.filter('words != Null or words != ""')

# Stopwords
x = df_ulysses_cleaned.except(df_stopwords)


x = spark.sql("SELECT words FROM df_ulysses_cleaned LEFT JOIN df_stopwords ON df_ulysses_cleaned.words = df_stopwords.words WHERE df_stopwords.value IS NULL")



df = sqlContext.sql(s"select * from learning.people where interest in $interest")

df_stopwords = df_stopwords.alias("words")



df_stopwords.show()

x = df_ulysses_cleaned.join(df_stopwords, ["words", "value"])



df = df_ulysses_cleaned.join(df_stopwords, df_ulysses_cleaned.words == df_stopwords.value)

df.show()
help(df_ulysses_cleaned.join)
df_ulysses_cleaned = df_stopwords.join(df_stopwords, df_ulysses_cleaned("words") === df_stopwords("value"))
...
df_ulysses_cleaned.words.regexp_replace(df_ulysses_cleaned['words'],, '')
